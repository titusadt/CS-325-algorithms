...
"""
def findMin(vertice_num, key, trees):
    minimum = 999999
    min_num =0
    for sure in range(vertice_num):
...
"""
def prims(vertice_num, distance, total_cases):
    #value to get minimum weight
    key = [999999] * vertice_num
    #array to store the spannning tree
    y = [0] * vertice_num
    #make sure to pick the first vertex
    key[0]=0
    trees = [False] * vertice_num
    #get the first node
    y[0] = -1

    for num in range(vertice_num):
        get = findMin(vertice_num, key, trees)
        trees[get] = True
        for v in range(vertice_num):
            if distance[get][v] > 0 and trees[v] == False and key[v] > distance[get][v]:
                key[v] = distance[get][v]
                y[v] = get
    findWeight(vertice_num, y, distance)   
    return(sum(key))
...
"""
def findWeight(vertice_num, y, distance):
    weight = 0
    for index in range(vertice_num):
        weight += distance[index][y[index]]
               
"""
Main function to open the file and apppend all of the values
"""
def main():
    fileName = open("graph.txt", "r")
    #getting the total number of test cases
    
    total_cases = fileName.readline()

    for cases in range(int(total_cases)):
        #this stores the total number of vertices in a test case
        vertice_num = int(fileName.readline())
        
        graph =[]
        graph_nodes =[]
        for T in range(vertice_num):
            nodes = fileName.readline()
            [x, y] = [int(j) for j in nodes.split(" ")]
            graph_nodes.append([x,y])
            #finding the distance here

        distance = []
        i = 0
        #creating and assigning all of the nodes
        for i in range(int(vertice_num)):
            temp = []
            j = i+1
            for j in range(int(vertice_num)):
                x1 = graph_nodes[i][0]
                x2 = graph_nodes[j][0]
                y1 = graph_nodes[i][1]
                y2 = graph_nodes[j][1]
        ...
        #Getting the spanning tree and printing the values
        ans = prims(vertice_num, distance, total_cases)
        print("Test case", cases+1, ": MST weight ", ans, "\n")
